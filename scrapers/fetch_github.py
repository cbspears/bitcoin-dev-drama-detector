"""
GitHub Scraper for Bitcoin Dev Drama Detector

Fetches recent activity from the bitcoin/bitcoin repository:
...
class GitHubScraper:
    """Scraper for bitcoin/bitcoin GitHub repository."""
    ...
    def fetch_pull_requests(self, since: datetime) -> List[dict]:
        ...
        """
        logger.info(f"Fetching PRs updated since {since}")
        ...
        for pr in self._paginate(endpoint, params):
            ...
            prs.append(pr_data)
            logger.debug(f"Fetched PR #{pr['number']}: {pr['title'][:50]}")
    ...
    def fetch_issues(self, since: datetime) -> List[dict]:
        ...
        """
        logger.info(f"Fetching issues updated since {since}")
        ...
        for issue in self._paginate(endpoint, params):
            ...
            logger.debug(f"Fetched issue #{issue['number']}: {issue['title'][:50]}")
    ...
    def fetch_all(self, days_back: int = 1) -> dict:
        """
        Fetch all GitHub data for the specified time period.
        ...
        logger.info(f"Fetching GitHub data from {since} to {until}")
        ...
        logger.info(f"GitHub fetch complete: {data['summary']}")
        return data


def main():
    """Main entry point for GitHub scraper."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Fetch GitHub data for Bitcoin Dev Drama Detector')
